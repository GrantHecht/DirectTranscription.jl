...
# Parameter struct
mutable struct DynamicsParams
    Tmax_us::Float64
end

# Instantiate parameter struct
ps          = DynamicsParams(1.5)

# ===== Define optimal control problem functions

# CR3BP dynamics with control
function CR3BPDynamicsWithHomotopy!(out, xVec, uVec, pVec, t, ps)
    # Constant parameters
    ...
    Tmax    = ps.Tmax_us*TU*TU / (MU*LU*1000.0)
    Isp     = 2000.0
...
function CR3BPDynamicsStateJacWithHomotopy!(out, xVec, uVec, pVec, t, ps)
    # Constant parameters
    μ       = 0.012150603793221306
    TU      = 3.75162997e5;
    LU      = 3.844e5;
    ...
    # Compute G 
    G11     = 1 - (1 - μ)*invr13 + 3*(1 - μ)*(x + μ)^2*invr15 - 
                μ*invr23 + 3*μ*(x + μ - 1)^2*invr25;
    G22     = 1 - (1 - μ)*invr13 + 3*(1 - μ)*y^2*invr15 - 
                μ*invr23 + 3*μ*y^2*invr25;
    G33     = -(1 - μ)*invr13 + 3*(1 - μ)*z^2*invr15 - 
                μ*invr23 + 3*μ*z^2*invr25;
    G12     = 3*(1 - μ)*(x + μ)*y*invr15 + 
                3*μ*(x + μ - 1)*y*invr25;
    G13     = 3*(1 - μ)*(x + μ)*z*invr15 + 
                3*μ*(x + μ - 1)*z*invr25;
    G23     = 3*(1 - μ)*y*z*invr15 + 
                3*μ*y*z*invr25;
    ...
    out[4, 5]   = 2.0
    out[5, 4]   = -2.0
    out[4, 7]   = -u*Tmax*ax/m^2
    out[5, 7]   = -u*Tmax*ay/m^2
    out[6, 7]   = -u*Tmax*az/m^2

    return nothing
end

stateJacSP = zeros(7, 7)
stateJacSP[1, 4] = 1.0
stateJacSP[2, 5] = 1.0
stateJacSP[3, 6] = 1.0
stateJacSP[4, 5] = 1.0
stateJacSP[5, 4] = 1.0
@views stateJacSP[4:6, 1:3] .= 1.0
@views stateJacSP[4:6, 7] .= 1.0

function CR3BPDynamicsControlJacWithHomotopy!(out, xVec, uVec, pVec, t, ps)
    # Constant parameters
    μ       = 0.012150603793221306
    TU      = 3.75162997e5;
    LU      = 3.844e5;
...
controlJacSP = zeros(7, 4)
controlJacSP[4, 1] = 1.0
controlJacSP[5, 2] = 1.0
controlJacSP[6, 3] = 1.0
@views controlJacSP[4:7, 4] .= 1.0

CR3BPDynamics!(out, xVec, uVec, pVec, t) = CR3BPDynamicsWithHomotopy!(out, xVec, uVec, pVec, t, ps)
CR3BPStateJac!(out, xVec, uVec, pVec, t) = CR3BPDynamicsStateJacWithHomotopy!(out, xVec, uVec, pVec, t, ps)
CR3BPControlJac!(out, xVec, uVec, pVec, t) = CR3BPDynamicsControlJacWithHomotopy!(out, xVec, uVec, pVec, t, ps)
...
dynFunc    = PathFunction(Dynamics(), CR3BPDynamics!, CR3BPStateJac!,
                CR3BPControlJac!, nothing, nothing, 7, 7, 4, 0, 
                stateJacSP, controlJacSP, zeros(7, 0), zeros(7, 2))
...
#DirectTranscription.SetOptions!(traj.solver, Dict("Iterations Limit" => 10))
Optimize!(traj)
sol = GetSolution(traj)
#sol = traj.data.phaseSet.pt[1].decisionVector.decisionVector
